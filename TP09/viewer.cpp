...
void Viewer::createFBO2() {
  // FBOs
  glGenFramebuffers(1,&_fboShading);

  // Textures
  glDeleteTextures(1,&_texBuff1);
  glDeleteTextures(1,&_texBuff2);
  glDeleteTextures(1,&_texBuff3);
...
void Viewer::updateFBO() {
  ...
  updateTex(_texShadow,GL_LINEAR,GL_CLAMP_TO_EDGE,_ndResol,_ndResol,GL_RGBA32F,GL_RGBA);
  ...
  //ajouter texture depth
  glBindTexture(GL_TEXTURE_2D,_texShadow);
  glFramebufferTexture2D(GL_FRAMEBUFFER_EXT,GL_COLOR_ATTACHMENT1,GL_TEXTURE_2D,_texShadow,0);
...
void Viewer::updateFBO2() {

  // create the noise texture
  updateTex(_texBuff1,GL_LINEAR,GL_CLAMP_TO_EDGE,_ndResol,_ndResol,GL_RGBA32F,GL_RGBA);
  updateTex(_texBuff2,GL_LINEAR,GL_CLAMP_TO_EDGE,_ndResol,_ndResol,GL_RGBA32F,GL_RGBA);
  updateTex(_texBuff3,GL_LINEAR,GL_CLAMP_TO_EDGE,_ndResol,_ndResol,GL_RGBA32F,GL_RGBA);
  updateTex(_texDepth,GL_LINEAR,GL_CLAMP_TO_EDGE,_ndResol,_ndResol,GL_DEPTH_COMPONENT24,GL_DEPTH_COMPONENT);

  // attach textures to the FBO dedicated to creating the terrain
  glBindFramebuffer(GL_FRAMEBUFFER,_fboShading);

  glBindTexture(GL_TEXTURE_2D,_texBuff1);
  glFramebufferTexture2D(GL_FRAMEBUFFER_EXT,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D,_texBuff1,0);

  glBindTexture(GL_TEXTURE_2D,_texBuff2);
  glFramebufferTexture2D(GL_FRAMEBUFFER_EXT,GL_COLOR_ATTACHMENT1,GL_TEXTURE_2D,_texBuff2,0);

  glBindTexture(GL_TEXTURE_2D,_texBuff3);
  glFramebufferTexture2D(GL_FRAMEBUFFER_EXT,GL_COLOR_ATTACHMENT2,GL_TEXTURE_2D,_texBuff3,0);
...
void Viewer::createShaders() {
  ...
  //full shader = 6
  _vertexFilenames.push_back("../TP09/shaders/gbuffer.vert");
  _fragmentFilenames.push_back("../TP09/shaders/gbuffer.frag");
  // ******************************


  //full shader = 7
  _vertexFilenames.push_back("../TP09/shaders/final.vert");
  _fragmentFilenames.push_back("../TP09/shaders/final.frag");
  // ******************************
...
//    glUniform1i(glGetUniformLocation(id,"neigeTex"),4);
    ...
    glActiveTexture(GL_TEXTURE0);
       glBindTexture(GL_TEXTURE_2D,_texBuff1);
       glUniform1i(glGetUniformLocation(id,"normal"),0);

      glActiveTexture(GL_TEXTURE1);
      glBindTexture(GL_TEXTURE_2D,_texBuff2);
      glUniform1i(glGetUniformLocation(id,"couleur"),1);

      glActiveTexture(GL_TEXTURE2);
      glBindTexture(GL_TEXTURE_2D,_texBuff3);
      glUniform1i(glGetUniformLocation(id,"lumiere"),2);


      glActiveTexture(GL_TEXTURE3);
      glBindTexture(GL_TEXTURE_2D,_texHeight);
      glUniform1i(glGetUniformLocation(id,"terrain"),3);
...
void Viewer::testShowTerrain(GLuint id) {
    ...
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D,_rocheTexId);
    glUniform1i(glGetUniformLocation(id,"coul"),1);
...
//  //TOujours dans _fboTerrain pour shadowmap
...
//  glDepthMask(GL_FALSE);
  ...
  glUseProgram(_shaders[6]->id());
  testShowTerrain(_shaders[6]->id());
  //active profondeur
...
//  glClear(GL_DEPTH_BUFFER_BIT);
 ...
 // glUseProgram(_shaders[7]->id());
 // testShowTerrain(_shaders[7]->id());
  ...
  glUseProgram(_shaders[7]->id());
  renderFinalImage(_shaders[7]->id());


  // *** show the generated height field (to be replaced) *** 

 // render in the viewport now A LA FIN DU PASS 1
 // glBindFramebuffer(GL_FRAMEBUFFER,0);

  // restore viewport sizes 
//  glViewport(0,0,width(),height());

  // clear buffers //DEBUT PASS 2
 // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
